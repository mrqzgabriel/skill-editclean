...
def build_ass(plan, out_path):
    """Gera o arquivo .ass com revelacao palavra a palavra e destaque por familia."""
    caps = plan["captions"]
    W = plan["output"]["width"]
    ...
    fsize = int(caps.get("font_size_px") or max(16, round(H * 0.038)))
    lheight = int(caps.get("line_height_px") or round(fsize * 1.21))
    ...
    # A enfase e feita por troca de FAMILIA tipografica, nao por cor: o accent
    # e branco igual ao resto, salvo se o plano pedir explicitamente outra cor.
    primary = _ass_color(caps.get("primary_hex", "#FFFFFF"))
    accent_col = _ass_color(caps.get("accent_hex", "#FFFFFF"))
    ...
    for name, fontname, bold, italic, colour in (
        ...
        lines.append(
            "Style: %s,%s,%d,%s,%s,%s,%s,%d,%d,0,0,100,100,-1,0,1,%.1f,%.1f,8,%d,%d,0,1"
            % (name, fontname, fsize, colour, colour, _ass_color("#000000"), back,
               bold, italic, outline_w, shadow_depth, margin_lr, margin_lr)
        )
    ...
    for blk in caps.get("blocks", []):
        ...
        if not words:
            continue
        anchor_key = blk.get("anchor", "lower_default")
        y_pct = anchors.get(anchor_key, 0.59)
        pos_y = int(H * y_pct)
        pos_x = W // 2
        n_lines = max(1, int(blk.get("lines", 1)))

        # cada palavra "acende" no seu tempo: emitimos um evento por estado
        # acumulado, mantendo as ja reveladas visiveis (pop-on tipo karaoke).
        for i, w in enumerate(words):
            ...
            for j, w2 in enumerate(words):
                ...
                parts_by_line.setdefault(ln, []).append((st, _ass_escape(w2["text"])))
            ...
            for ln in range(n_lines):
                if ln not in parts_by_line:
                    continue
                seg = []
                for st, txt in parts_by_line[ln]:
                    if st == "Normal":
                        seg.append("{\\rNormal}%s" % txt)
                    elif st == "Strong":
                        seg.append("{\\rStrong}%s" % txt)
                    else:
                        seg.append("{\\rAccent}%s" % txt)
            ...
            fade_tag = "{\\fad(%d,0)}" % fade if fade > 0 else ""
            pos_tag = "{\\an8\\pos(%d,%d)}" % (pos_x, pos_y)

            lines.append(
                "Dialogue: 0,%s,%s,Normal,,0,0,0,,%s%s%s"
                % (_ass_time(start), _ass_time(end), pos_tag, fade_tag, text)
            )
...
def build_zoom_chain(seg, label_in, label_out, W, H, fps):
    ...
    return [
        "[%s]zoompan=z='%s':x='(iw-iw/zoom)*%.4f':y='(ih-ih/zoom)*%.4f':"
        "d=1:s=%dx%d:fps=%s,setsar=1[%s]"
        % (label_in, zexpr, ax, ay, W, H, fps, label_out)
    ]
...
def build_filtergraph(plan, ass_path=None, overlay_inputs=None):
    ...
    if op.get("enabled") and op.get("type") == "blur_zoom_out":
        ...
        parts.append(
            ...
            "d=1:s=%dx%d:fps=%s,setsar=1[%s_c]"
            % (cur_v, zexpr, W, H, fps, nv)
        )
